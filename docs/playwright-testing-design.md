...
- The fixture waits for:
  ...
  - relevant images to finish loading
...
The post-deploy live job waits for the custom domain to respond before running `PLAYWRIGHT_BASE_URL=https://alexleung.ca` through the same Dockerized smoke service. This catches production-only pathing, Pages, or custom-domain regressions that a local export check cannot see.
